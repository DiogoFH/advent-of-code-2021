...
def solver():
    ...
    paths = explore(nodeList['start'], [], False)
    print(paths)
...
def explore(node, currentPath, visitedSmallCaveTwice):
    ...
    for a in node.adjacencyList:
        if a.isSmall and isInPath(a, currentPath):
            if visitedSmallCaveTwice:
                ...
                paths += explore(a, currentPath, True)
        else:
            paths += explore(a, currentPath, visitedSmallCaveTwice)
    currentPath.pop()
    return paths
